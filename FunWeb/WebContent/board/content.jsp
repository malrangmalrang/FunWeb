...
<%
	...
	BoardVO board = bdao.getBoard(num);
...
<table class="greyGridTable">
	...
	<tr>
		...
		<%if(board.getFile() != null) { %>
			<img src="../board_upload/<%=board.getFile() %>" alt="<%=board.getFile() %>" width="600px";><br>
		<%} %>
		<%=content%></td>
	</tr>
	<tr>
		<td colspan="4" style="text-align: center;">
			...
			<% 
				...
				if(id != null) { %>
					<input type="button" value="답글" onclick="location.href='writeRe.jsp?re_ref=<%=board.getRe_ref()%>&re_lev=<%=board.getRe_lev()%>&re_seq=<%=board.getRe_seq()%>'">
