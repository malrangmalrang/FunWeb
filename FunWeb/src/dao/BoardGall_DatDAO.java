...
public class BoardGall_DatDAO {
	
	private Connection con;
	private PreparedStatement stmt;
	private ResultSet rs;
	private static BoardGall_DatDAO bddao;
	
	private BoardGall_DatDAO() {}
	
	public static BoardGall_DatDAO getInstance() {
		if(bddao == null) {
			bddao = new BoardGall_DatDAO();
		}
		return bddao;
	}
	...
	public int insertDat(DatVO dat) {
		int check = 0;
		int max = 0;
		String sql1 = "SELECT MAX(dat_num) FROM board_gall_dat WHERE board_num=?";
		String sql2 = "INSERT INTO board_gall_dat VALUES(?, ?, ?, ?, now(), ?, ?, ?)";
	...
	public int insertReDat(DatVO dat) {
		...
		int max = 0;
		String sql1 = "SELECT MAX(dat_num) FROM board_gall_dat WHERE board_num=?";
		String sql2 = "UPDATE board_gall_dat SET dat_re_seq=dat_re_seq+1 WHERE board_num=? AND dat_re_ref=? AND dat_re_seq > ?";
		String sql3 = "INSERT INTO board_gall_dat VALUES(?, ?, ?, ?, now(), ?, ?, ?)";
		con = JdbcUtil.getConnection();
	...
	public int getDatCount(int board_num) {
		int count = 0;
		String sql = "SELECT COUNT(*) FROM board_gall_dat WHERE board_num=?";
		con = JdbcUtil.getConnection();
	...
	public List<DatVO> getDatList(int startRow, int pageSize, int board_num) {
		...
		String sql = "SELECT * FROM board_gall_dat WHERE board_num=? ORDER BY dat_re_ref ASC, dat_re_seq ASC LIMIT ?, ?";
		con = JdbcUtil.getConnection();
		try {
			stmt = con.prepareStatement(sql);
			stmt.setInt(1, board_num);
	...
	public int updateDat(int dat_num, int board_num, String dat_content) {
		int check = 0;
		String sql = "UPDATE board_gall_dat SET dat_content=? WHERE dat_num=? AND board_num=?";
		con = JdbcUtil.getConnection();
	...
	public int deleteDat(int dat_num, int board_num) {
		int check = 0;
		String sql = "DELETE FROM board_gall_dat WHERE dat_num=? AND board_num=?";
		con = JdbcUtil.getConnection();
